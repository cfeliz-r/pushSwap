...
/*   Updated: 2024/06/08 17:52:19 by cfeliz-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static t_towers	return_to_a_third(t_towers tower, int dir)
{
	if (dir == -1)
...
static t_towers	return_to_a_second(t_towers tower, int *min, int *max, int b)
{
	int		i;
	int		dir;
...
t_towers	move_back_to_a(t_towers tower, int *min, int *max)
{
	int		i;

	i = 0;
	while (tower.stk_b[i] != INT_MAX)
		i++;
	i--;
	while (tower.stk_b[0] != INT_MAX)
	{
